...
describe('Restaurant', () => {
    ...
    test('can create a restaurant', async () => {
        const restaurant = await Restaurant.create({name: "Padella", image: "image.url"})
        expect(restaurant.id).toBe(1)
    })

    test('can add a menu to a restaurant', async () => {
        const restaurant = await Restaurant.create({name: "Inamo", image: "image.url"})
...
describe('Menu', () => {
    ...
    test('can add a new item to a menu', async() =>{
        const menu = await Menu.create({title:'Starter Menu', image:'image.url'})
        const item = await Item.create({name: "Calamari", price: 11, image:" image.url"})
...
describe("Item", () => {
    ...
    test.skip("can create a new item", async() => {
        const item = await Item.create({name: "Steak", price: 19.95, image:" image.url"})
